...
const GetEmailOtpRequest = function* (data) {
  ...
  try {
    ...
    const requesUrl = `${Constants.BASE_URL}auth/signup-email-send-otp`;
...
const GetEmailOtpVerify = function* (data) {
  ...
  try {
    ...
    const requestUrl = `${Constants.BASE_URL}auth/signup-verify-otp`;
...
const fetchCountryCode = function* () {
  yield put(setIsLoading(true));
  try {
    const requestUrl = `${Constants.BASE_URL}get-all-countries-list`;
...
const GetPhoneOtp = function* (data) {
  ...
  try {
    ...
    let requestUrl = `${Constants.BASE_URL}auth/signup-phone-send-otp`;
    let res = yield call(GetRecord, requestUrl, postData);

    if (res?.data != null) {
      // console.log("show otp request",res)
      if (res.data.status == 201) {
        alert(res.data.message);
      } else {
        let otpdata = res.data.data.otp;
        console.log('show otp ', otpdata);
...
const getPhoneNumberVerify = function* (data) {
  try {
    let requestUrl = `${Constants.BASE_URL}auth/signup-verify-otp`;
    let postData = {
...
const handleSocialLogin = function* (data) {
  ...
  try {
    ...
    let requestUrl = `${Constants.BASE_URL}auth/social-login`;
...
const AccountDeleteSaga = function* () {
  // console.log("ENTER INOTO DELTE FUNCIOTN")
  const {displayNotification} = useNotificaiton();
  yield put(setIsLoading(true));
  try {
    let requestUrl = `${Constants.BASE_URL}user/delete`;
    const res = yield call(fetchRecord, requestUrl);
    if (res.status == 200) {
      console.log("res statsu",res)
      displayNotification('DELETE', 'Account deleted Successfully' + '');
      AsyncStorage.clear();
      // dispatch(Logout(false));
      yield put({type: actions.LOGOUT, payload:false});
    }
    yield put(setIsLoading(false));
  } catch (error) {
    alert(error);
    console.log("show erorrr",error)
    yield put(setIsLoading(false));
  }
};
...
const AuthSaga = [
  ...
  takeLatest(actions.DELETE_ACOUNT, AccountDeleteSaga),
];
